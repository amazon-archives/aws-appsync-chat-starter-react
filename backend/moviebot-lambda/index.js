        ...
        tmdb.search.movie( `${moviename}`, function(err,res) {
            if (err)
                ...
                if (res.errors){
                    ...
                    {'contentType': 'PlainText', 'content': 'Please try something like: [@moviebot Tell me about a movie], [@moviebot Tell me about [Movie]], [@moviebot [Movie] [Plot]], [@moviebot [Movie] [Year]] or even just [@moviebot [Movie]]'}));
                    ...
                    if  (whatInfo === 'Year' || whatInfo === 'Release Date' || whatInfo === 'release date'|| whatInfo === 'date'|| whatInfo === 'date'|| whatInfo === 'Release Year' || whatInfo === 'release year' || whatInfo === 'year'){
                        callback(close(sessionAttributes, 'Fulfilled',
                        {'contentType': 'PlainText', 'content': ` ${moviename} released in: ${resDate} - IMDB Link: ${resLink} - IMDB Poster: ${resPoster}`}));
                      }
                      else if (whatInfo === 'Plot' || whatInfo === 'Story' || whatInfo === 'plot' || whatInfo === 'story'){
                        callback(close(sessionAttributes, 'Fulfilled',
                        {'contentType': 'PlainText', 'content': `Plot of ${moviename} is: ${resPlot} - IMDB Link: ${resLink} - IMDB Poster: ${resPoster} - IMDB Poster: ${resPoster}`}));
                      }
                
                      else
                        callback(close(sessionAttributes, 'Fulfilled',
                        {'contentType': 'PlainText', 'content': `Movie Name: ${moviename}, Year: ${resDate}, Plot: ${resPlot} - IMDB Link: ${resLink} - IMDB Poster: ${resPoster}`}));
                });
                }
        });
}
